...
function journalHTML(){
  const S=G.S; let h='<button class="btn ghost close" id="jClose">Close</button><button class="btn ghost close" id="jOpts" style="margin-right:8px">⚙ Options</button><h1>Journal</h1><div class="sub">'+esc(GM.HERO.name+', '+GM.HERO.role)+'  ·  '+esc(GM.rank(S))+'  ·  '+esc(GM.clock(S))+'  ·  progress saves on its own</div>'+statsHTML(S);
...
/* chapter previews: a small drawn picture per card */
function chapterPreview(cv,id){ const c=cv.getContext('2d'), W=cv.width, H=cv.height, D=window.HDRAW, t=1.3; c.clearRect(0,0,W,H);
  const sky=(a,b)=>{ const g=c.createLinearGradient(0,0,0,H); g.addColorStop(0,a); g.addColorStop(1,b); c.fillStyle=g; c.fillRect(0,0,W,H); };
  const floor=col=>{ c.fillStyle=col; c.fillRect(0,H*0.78,W,H*0.22); };
  c.save();
  if(id==='cutover'){ sky('#2c4668','#e9a86a'); floor('#1d2430'); c.save(); c.translate(W*0.3,H*0.78); c.scale(0.55,0.55); D.aplusMonster(c,0,0,1,t,'mean',{dir:1,loom:1}); c.restore();
    c.fillStyle='#f0e0c0'; c.beginPath(); c.arc(W*0.72,H*0.42,H*0.24,0,7); c.fill(); c.strokeStyle='#3a2a1a'; c.lineWidth=2; c.stroke(); c.beginPath(); c.moveTo(W*0.72,H*0.42); c.lineTo(W*0.72,H*0.24); c.moveTo(W*0.72,H*0.42); c.lineTo(W*0.72+H*0.14,H*0.42); c.stroke(); c.fillStyle='#f2b544'; c.font='700 11px "IBM Plex Mono",monospace'; c.textAlign='center'; c.fillText('00:00',W*0.72,H*0.9); }
  if(id==='trivia'){ sky('#1c2a4a','#3a4a6a'); floor('#3a3e47'); for(let k=0;k<6;k++){ c.fillStyle=['#243447','#2f4a7f','#243447'][k%3]; c.fillRect(W*0.08+k*W*0.15,H*0.18,W*0.13,H*0.22); c.fillStyle='#f2b544'; c.font='700 10px "IBM Plex Mono",monospace'; c.textAlign='center'; c.fillText(String(200*(k+1)),W*0.145+k*W*0.15,H*0.32); }
    c.fillStyle='#5a3a24'; c.fillRect(W*0.62,H*0.5,W*0.22,H*0.3); c.fillStyle='#f6ecd8'; c.font='900 22px "IBM Plex Sans",sans-serif'; c.fillText('?',W*0.73,H*0.68); c.fillStyle='#7fe0a0'; c.font='700 9px "IBM Plex Mono",monospace'; c.fillText('FINAL JEOPARDY',W*0.3,H*0.72); }
  if(id==='aivs'){ sky('#3d4a5c','#26303e'); floor('#1d2430'); c.save(); c.translate(W*0.28,H*0.78); c.scale(0.5,0.5); D.aplusMonster(c,0,0,1,t,'confused',{dir:1}); c.restore();
    const x=W*0.72,y=H*0.42; c.fillStyle='#7fd0ff'; c.strokeStyle='#151a22'; c.lineWidth=1.5; c.beginPath(); c.roundRect(x-22,y-14,44,28,8); c.fill(); c.stroke(); c.fillStyle='#101a2e'; c.beginPath(); c.arc(x-7,y-3,2.5,0,7); c.arc(x+7,y-3,2.5,0,7); c.fill(); c.strokeStyle='#101a2e'; c.beginPath(); c.arc(x,y+2,5,0.2,Math.PI-0.2); c.stroke(); c.fillStyle='#f6ecd8'; c.font='700 10px "IBM Plex Sans",sans-serif'; c.textAlign='center'; c.fillText('Hi! What is an order?',x,y+28); }
  if(id==='loop'){ sky('#120e0a','#2a1e14'); for(let k=0;k<3;k++){ const r=H*0.18+k*H*0.1; c.strokeStyle='rgba(240,224,192,'+(0.9-k*0.3)+')'; c.lineWidth=2; c.beginPath(); c.arc(W*0.5,H*0.5,r,0.3+k*0.5,Math.PI*1.8+k*0.5); c.stroke(); }
    c.fillStyle='#f0e0c0'; c.beginPath(); c.arc(W*0.5,H*0.5,H*0.16,0,7); c.fill(); c.strokeStyle='#3a2a1a'; c.lineWidth=2; c.beginPath(); c.moveTo(W*0.5,H*0.5); c.lineTo(W*0.5,H*0.38); c.moveTo(W*0.5,H*0.5); c.lineTo(W*0.5-H*0.1,H*0.56); c.stroke();
    c.fillStyle='#f2b544'; c.font='700 12px "IBM Plex Mono",monospace'; c.textAlign='center'; c.fillText('6:00 PM  →  12:00 AM  →  6:00 PM',W*0.5,H*0.9); }
  if(id==='pinball'){ sky('#2b1436','#3a1a4a'); c.fillStyle='#1a1d24'; c.fillRect(W*0.15,H*0.12,W*0.7,H*0.3); for(let k=0;k<8;k++){ c.fillStyle=k%2?'#f2b544':'#e0563a'; c.beginPath(); c.arc(W*0.2+k*W*0.085,H*0.18,3,0,7); c.fill(); }
    c.fillStyle='#6fe08a'; c.font='700 14px "IBM Plex Mono",monospace'; c.textAlign='center'; c.fillText('1,985,000',W*0.5,H*0.34); c.fillStyle='#d8d2c0'; c.beginPath(); c.arc(W*0.62,H*0.62,6,0,7); c.fill(); for(const k of [-1,1]){ c.save(); c.translate(W*0.5+k*W*0.14,H*0.78); c.rotate(k*0.35); c.fillStyle='#f2b544'; c.beginPath(); c.roundRect(k<0?0:-30,-4,30,8,4); c.fill(); c.restore(); } }
  if(id==='raft'){ sky('#8fb8e0','#cfe3f0'); c.fillStyle='#3f6b3a'; c.beginPath(); c.moveTo(0,H*0.5); for(let x=0;x<=W;x+=20) c.lineTo(x,H*0.42-Math.sin(x*0.05)*8); c.lineTo(W,H*0.5); c.fill(); c.fillStyle='#2d6f9a'; c.fillRect(0,H*0.55,W,H*0.45); c.strokeStyle='rgba(255,255,255,.5)'; for(let k=0;k<8;k++){ c.beginPath(); c.moveTo(k*W*0.14,H*0.6+(k%3)*9); c.quadraticCurveTo(k*W*0.14+8,H*0.56+(k%3)*9,k*W*0.14+16,H*0.6+(k%3)*9); c.stroke(); }
    c.fillStyle='#6a6f78'; c.beginPath(); c.ellipse(W*0.8,H*0.82,12,8,0,0,7); c.fill(); c.fillStyle='#e0a030'; c.strokeStyle='#151a22'; c.lineWidth=1.5; c.beginPath(); c.ellipse(W*0.42,H*0.75,44,12,0,0,7); c.fill(); c.stroke(); c.fillStyle='#e0563a'; c.fillRect(W*0.42-40,H*0.75-18,14,12); for(let k=0;k<4;k++){ c.fillStyle=['#2f6f9f','#c25a3a','#3d6b52','#7a4a2a'][k]; c.beginPath(); c.arc(W*0.42-18+k*13,H*0.75-10,5,0,7); c.fill(); } }
  c.restore(); }
/* the chapter menu: one card per chapter, each with its own save */
function openChapters(){ const el=$('chapters'), list=$('chList'); let h='';
  for(const c of GM.CHAPTERS||[]){ const s=loadSave(c.id); h+='<div class="chap"><canvas class="prev" data-prev="'+c.id+'" width="180" height="100"></canvas><div class="num">CHAPTER '+c.num+'</div><h2>'+esc(c.title)+'</h2><p>'+esc(c.blurb)+'</p><div class="goal">'+esc(c.goal)+'</div>'+(s?'<div class="prog">Saved: '+GM.percent(s)+'%, '+esc(GM.rank(s))+'</div>':'')+'<div class="btns">'+(s?'<button class="btn" data-ch="'+c.id+'" data-mode="continue">Continue</button><button class="btn ghost" data-ch="'+c.id+'" data-mode="new">Start over</button>':'<button class="btn" data-ch="'+c.id+'" data-mode="new">Play</button>')+'</div></div>'; }
  list.innerHTML=h; for(const cv of list.querySelectorAll('canvas[data-prev]')){ try{ chapterPreview(cv,cv.dataset.prev); }catch(_){ } } el.classList.remove('hide'); $('title').classList.add('hide'); state='chapters';
