...
function renderShow(c,V,G,now){
  ...
  EPI.BOOTHS.forEach((b,i)=>{ const x=EPI.BX(i); R(x,-90,200,90,COLS[i]); R(x,-90,200,6,'rgba(255,255,255,.15)'); c.fillStyle='#f6ecd8'; c.font='700 9px "IBM Plex Sans Condensed","IBM Plex Sans",sans-serif'; c.fillText(b,x+100,-76);
    ...
    if(b==='IT DEPT'){ const tx=x+60; if(D.aplusMonster) D.aplusMonster(c,tx,0,0.62,t,'quiet',{dir:1}); c.save(); c.translate(tx,-30); c.rotate(-0.5); c.fillStyle='#c0392b'; c.fillRect(-2,-16,4,32); c.restore(); c.fillStyle='#243447'; c.font='600 4px "IBM Plex Sans",sans-serif'; c.fillText('A+  ·  1985–2026',tx+34,-40); c.fillText('archived with honors',tx+34,-35); if(D.PROPS.pinball) D.PROPS.pinball(x+160,0,now,c); }
