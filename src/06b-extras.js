...
function startEnd(){ const G=H.G, S=G.S; if(GM.chapterEnding&&GM.chapterEnding(S)){ if(S.runEnd==null) S.runEnd=S.run||0; H.showEnding(); return; } if(S.runEnd==null) S.runEnd=S.run||0; try{ const b=JSON.parse(localStorage.getItem(BEST_KEY)||'null'); if(b==null||S.runEnd<b) localStorage.setItem(BEST_KEY,JSON.stringify(S.runEnd)); }catch(_){ }
