...
function pose(x,t,F,kind,o){ o=o||{};
  ...
  const K={run:{f:1.6,th:0.95,kb:0.35,ka:1.55,lean:0.32,arm:1.15,el:1.75,bob:3.8},scramble:{f:5.5,th:1.05,kb:0.5,ka:1.4,lean:0.12,arm:1.3,el:1.5,bob:1.2},
    ...
    fold:{f:0.3,th:0,kb:0.08,ka:0,lean:-0.05,arm:0,el:2.2,bob:0},bow:{f:0.3,th:0,kb:0.15,ka:0,lean:0,arm:0,el:0.4,bob:0},kick:{f:0,th:0,kb:0.05,ka:0,lean:-0.06,arm:0,el:0.15,bob:0}}[kind]||{};
  ...
  for(let i=0;i<2;i++){ const p=ph+i*Math.PI, th=kind==='kick'?1.45*Math.pow(Math.max(0,Math.sin(kp+i*Math.PI)),2):K.th*Math.sin(p)+(kind==='stand'||kind==='fold'||kind==='bow'?(i?0.06:-0.06):0), k=kind==='kick'?0.05:K.kb+K.ka*Math.max(0,Math.sin(p+0.9));
  ...
  const hipY=-(AH+Math.max(lows[0],lows[1]))-K.bob*(0.5+0.5*Math.cos(2*ph))-(o.hop||0)-(kind==='kick'?2.5*Math.abs(Math.sin(kp)):0)+(kind==='stand'||kind==='fold'?Math.sin(t*2.1)*0.3:0);
